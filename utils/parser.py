...
class Parser(object):
    ...
    def get_parser(self):
        ...
        # config file
        parser.add_argument(
            "--config", type=str, required=True, help="Config name.", action=cmdAction
        )
        parser.add_argument(
            "--exp_name",
            type=str,
        ...
        parser.add_argument(
            "--phase", type=str, default="train", help="train or test", action=cmdAction
        )
        parser.add_argument(
            "--phase_task",
            type=str,
            default=None,
            help="loss_landscape or others",
            action=cmdAction,
        )
        # task order: list of int
        parser.add_argument(
            "--task_order", type=int, nargs="+", default=None, help="task order"
        )
        ...
        parser.add_argument(
            "--local_rank", type=int, default=-1, help="dist ddp", action=cmdAction
        )
        parser.add_argument(
            "--user", type=str, default="ubuntu", help="user", action=cmdAction
        )
        ...
        parser.add_argument(
            "--lr", type=float, default=0.1, help="Learning rate.", action=cmdAction
        )
        ...
        parser.add_argument(
            "--n_epochs", type=int, default=20, help="Training epoch.", action=cmdAction
        )
        parser.add_argument(
            "--n_tasks", type=int, default=5, help="Task number.", action=cmdAction
        )
        parser.add_argument(
            "--batch_size", type=int, default=128, help="Batch size.", action=cmdAction
        )
        parser.add_argument(
            "--num_workers",
            type=int,
        ...
        parser.add_argument(
            "--seed", type=int, default=1024, help="The random seed.", action=cmdAction
        )
        # online args
        parser.add_argument(
            "--online",
        ...
        parser.add_argument(
            "--noise_level", type=int, default=10, help="Noise level.", action=cmdAction
        )
    ...
    def set_exp_dir(self):
        ...
        if self.args.exp_name == "default":
            ...
            if self.args.weight_path is not None and "joint" in self.args.model:
                self.args.exp_name = (
                    self.args.model + f"{self.args.n_tasks:02d}-pretrain_backbone"
                )
            ...
            for k, v in param_groups.items():
                if hasattr(self.args, f"{k}_non_default") and v != "" and v is not None:
                    self.args.exp_name += v

            # task order
            if self.args.task_order is not None:
                self.args.exp_name += "-order" + "".join(
                    [str(i) for i in self.args.task_order[1:]]
                )

        # exp_dir: ./outputs/user-fscl/dataset/exp_name
        self.args.exp_dir = os.path.join(
            "./outputs",
            self.args.user + "-fscl" if self.args.fewshot else self.args.user,
            self.args.dataset + "-online" if self.args.online else self.args.dataset,
            self.args.exp_name,
        )
        ...
        # if seven in exp_dir, replace it with seven{:2d} self.args.dataset_args['class_idx]
        if "seven" in self.args.exp_dir and hasattr(self.args, "dataset_args"):
            class_idx = self.args.dataset_args["class_idx"]
            if class_idx is not None:
                self.args.exp_dir = self.args.exp_dir.replace(
                    "seven", f"seven{class_idx:02d}"
                )

        os.makedirs(self.args.exp_dir, exist_ok=True)

    def set_device(self):
        # set device: if local_rank is set, use it; otherwise, use the first gpu or cpu
        self.args.local_rank = int(os.environ.get("LOCAL_RANK", "-1"))

        if self.args.local_rank != -1:
            # distributed training
            if (
                len(self.args.gpus) > self.args.local_rank
            ):  # use gpus to control the local rank
                self.args.local_rank = self.args.gpus[self.args.local_rank]
            torch.cuda.set_device(self.args.local_rank)
            torch.distributed.init_process_group(backend="nccl")
            self.args.output_device = torch.device(f"cuda:{self.args.local_rank}")
        elif (
            self.args.gpus == [] or not torch.cuda.is_available()
        ):  # no gpus or cuda not available
            self.args.output_device = torch.device("cpu")
        else:  # use the first gpu as output device
    ...
    def check_args(self):
        ...
        # task order, legend: 1~5 for 5 tasks
        if self.args.task_order is not None:
            assert (
                len(self.args.task_order) == self.args.n_tasks
            ), "task order length must be equal to n_tasks"
            assert set(self.args.task_order) == set(
                [i + 1 for i in range(self.args.n_tasks)]
            ), "task order must contain all tasks"

            self.args.task_order.insert(0, 0)  # add a dummy task at the beginning
        ...
        self.args.log_file = os.path.join(
            self.args.log_dir, f"{self.args.phase}-{self.args.timestamp}.log"
        )
        # gpus
        self.set_device()

        # set full model pretrain weight path
        ...
        # set default backbone weight path
        ...
        # set weight save path
        self.args.weight_save_path = os.path.join(
            self.args.exp_dir, f"weights/weight.pth"
        )
    ...
    def reuse_config(self):
        ...
        if self.args.phase == "train" and self.args.pretrain:
            ...
            for k, v in old_args.items():
                if k in vars(self.args).keys() and not hasattr(
                    self.args, f"{k}_non_default"
                ):
    ...
    def back_proj(self):
        # save the configuration file (yaml)
        config = os.path.join(self.args.exp_dir, f"configs/{self.args.phase}.yaml")
        os.makedirs(os.path.dirname(config), exist_ok=True)
