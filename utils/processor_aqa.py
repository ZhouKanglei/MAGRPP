...
from utils.loss_landscape import calculate_loss
...
class ProcessorAQA(Processor):
    ...
    def __init__(self, args):
        ...
        self.best_weight_path = os.path.join(
            os.path.dirname(self.args.weight_save_path), "best.pth"
        )
        self.last_weight_path = os.path.join(
            os.path.dirname(self.args.weight_save_path), "last.pth"
        )
    ...
    def load_best_model(self, weight_path=None):
        if weight_path is None and self.args.phase == "train":
            return
        elif weight_path is None:
            weight_path = self.args.weight_save_path.replace("weight.pth", "best.pth")

        if os.path.exists(weight_path):
            state = torch.load(weight_path, map_location=self.args.output_device)
            ...
            weight_path = weight_path.replace(
                "best.pth", f"last-task{self.args.n_tasks}.pth"
            )
            ...
            if os.path.exists(weight_path):
                ...
                self.model.load_state_dict(state["model"], strict=False)
                self.args.logging.info(
                    f"Load pretrained model state from {weight_path}."
                )
                self.args.logging.info(
                    "Current: \n" + pprint.pformat(state["current_metrics"])
                )
    ...
    def save_checkpoint(self, epoch, metrics, filename="last"):
        ...
        checkpoint_file = (
            self.best_weight_path if filename == "best" else self.last_weight_path
        )
    ...
    def save_best(self):
        if self.args.local_rank < 1:
            self.args.logging.info(
                f"New best found (e{self.current_epoch}):\n"
    ...
    def evaluate(self, dataset, last=False):
        ...
        if "joint" in self.model.NAME:
            dataset.test_loaders = [dataset.get_joint_loader("test")]
        ...
        for k, test_loader in enumerate(dataset.test_loaders):
            ...
            for data in test_loader:
                with torch.no_grad():
                    inputs, labels = data
                    inputs, labels = inputs.to(self.args.output_device), labels.to(
            ...
            self.args.logging.info(
                f"Evaluate T{k + 1:02d}, {len(output_scores):3d} samples, "
            ...
            self.all_label_scores, self.all_output_scores = (
                all_label_scores,
                all_output_scores,
            )

            if set(all_label_scores) != set(label_scores):
                rho, p, L2, RL2 = self.compute_metric(
                    all_output_scores, all_label_scores
                )
                self.args.logging.info(
                    f"      T01-{k + 1:02d}, {len(all_output_scores):3d} samples, "
                    f"rho:{rho:6.2f} %, p: {p:5.4f}, L2: {L2:7.2f}, RL2: {RL2:8.2f}"
                )
    ...
    def training_loop(self, train_loader, progress_bar, scheduler, t):
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if self.args.local_rank is not -1:
            ...
            for i, data in enumerate(train_loader):
                ...
                if hasattr(train_loader.dataset, "logits"):
                    ...
                    loss = self.model.meta_observe(
                        inputs, labels, not_aug_inputs, epoch, t, logits=logits
                    )
                else:
    ...
    def train(self):
        ...
        if "joint" not in self.model.NAME:
            ...
            self.random_results_class, self.random_results_task = self.evaluate(
                dataset_copy
            )
        # trained model
        ...
        for t in range(self.dataset.N_TASKS):
            ...
            if (
                t
                and self.dataset.SETTING != "domain-il"
                and "joint" not in self.model.NAME
            ):
                self.evaluate(self.dataset, last=True)
                self.results[t - 1] = self.results[t - 1] + self.rhos
                if self.dataset.SETTING == "class-il":
                    self.results_mask_classes[t - 1] = (
                        self.results_mask_classes[t - 1] + self.rhos_mask_classes
                    )

            # middle task
            if (
                os.path.exists(self.args.base_pretrain_model_path)
                and t == 0
                and self.args.base_pretrain
                and "joint" not in self.model.NAME
            ):
    ...
    def task_stat(self, logger):
        ...
        self.save_checkpoint(
            -1, self.current_metrics, filename=f"last-task{self.current_task}"
        )
    ...
    def final_stat(self, logger):
        ...
        if self.dataset.SETTING != "domain-il" and "joint" not in self.model.NAME:
            ...
            if self.model.NAME not in ["icarl", "pnn"]:
                ...
                logger.add_fwt(
                    self.results,
                    self.random_results_class,
                    self.results_mask_classes,
                    self.random_results_task,
                )
        # log
    ...
    def test_w_memory_feats(self):
        save_dir = os.path.join(self.args.exp_dir, "results")
        os.makedirs(save_dir, exist_ok=True)
        # go to the last task
        for t in range(self.dataset.N_TASKS):

            weight_path = self.args.weight_save_path.replace(
                "weight.pth", f"last-task{t}.pth"
            )
            self.load_best_model(weight_path=weight_path)
            ...
            # feature
            self.model.end_task(self.dataset)
            buf_x, buf_lab, buf_tl = self.model.buffer.get_all_data()
            num_x = self.model.buffer.num_seen_examples
            print("-----------------", num_x)

            feature_filename = os.path.join(save_dir, f"feature_task{t}.npz")
            np.savez(
                feature_filename,
                features=buf_x[:num_x],
                labels=buf_lab[:num_x],
                task_ids=buf_tl[:num_x],
            )
            self.args.logging.info(f"Save file to {feature_filename}.")

    def test_w_sample_feats(self):
        save_dir = os.path.join(self.args.exp_dir, "results")
        os.makedirs(save_dir, exist_ok=True)
        # go to the last task
        for t in range(self.dataset.N_TASKS):
            ...
            features, scores, task_ids = [], [], []
            for tt in range(1, len(self.dataset.train_loaders)):
                train_loader = self.dataset.train_loaders[tt]
                ...
                for i, data in enumerate(train_loader):
                    ...
                    feats = self.model.module.feature_extractor(inputs)

                    feats = distributed_concat(feats)
                    labels = distributed_concat(labels)

                    features.append(feats.detach().cpu().numpy())
                    scores.append(labels.detach().cpu().numpy())
                    task_ids.append(np.array([tt] * len(labels)))

            features = np.concatenate(features, axis=0)
            scores = np.concatenate(scores, axis=0)
            task_ids = np.concatenate(task_ids, axis=0)

            feature_filename = os.path.join(save_dir, f"sample_feature_task{t}.npz")
            np.savez(
                feature_filename,
                features=features,
                labels=scores,
    ...
    def test_w_loss_landascape(self):
        save_dir = os.path.join(self.args.exp_dir, "results")
        os.makedirs(save_dir, exist_ok=True)
        # go to the last task
        for t in range(self.dataset.N_TASKS):
            ...
            steps = np.linspace(-0.5, 0.5, num=20)
            ...
            calculate_loss(
                ...
                steps,
                dir_num=10,
                output_dir=self.args.exp_dir,
            )

    def test_w_optimal_layer(self):
        save_dir = os.path.join(self.args.exp_dir, "layers")
        os.makedirs(save_dir, exist_ok=True)
        # go to the first task
        self.model.net.eval()
        _, _ = self.dataset.get_data_loaders()

        test_loader = self.dataset.test_loaders[0]

        old_encoder = copy.deepcopy(self.model.module.feature_extractor)

        self.load_best_model(weight_path=self.args.weight_path)
        new_encoder = copy.deepcopy(self.model.module.feature_extractor)

        layer_keys = self.model.module.feature_extractor.backbone.layer_keys()

        for cnt, layer in enumerate(layer_keys):
            all_labels, all_old_feats, all_new_feats = [], [], []
            ...
            for i, data in enumerate(test_loader):
                with torch.no_grad():
                    ...
                    old_feats, _ = old_encoder(inputs, True)
                    new_feats, _ = new_encoder(inputs, True)

                    if self.args.local_rank != -1:
                        labels = distributed_concat(labels)
                        old_feats = distributed_concat(old_feats[layer])
                        new_feats = distributed_concat(new_feats[layer])
            ...
            cnt += 1
            all_old_feats = torch.cat(all_old_feats, dim=0).cpu().numpy()
            all_new_feats = torch.cat(all_new_feats, dim=0).cpu().numpy()

            feature_filename = os.path.join(save_dir, f"{cnt:02d}.npz")
            np.savez(
                feature_filename,
                labels=all_labels,
                old_features=all_old_feats,
                new_features=all_new_feats,
            )
            self.args.logging.info(f"Save file to {feature_filename}.")

    def test_w_case_study(self):
        save_dir = os.path.join(self.args.exp_dir, "results")
        os.makedirs(save_dir, exist_ok=True)
        ...
        score_filename = os.path.join(save_dir, f"score_task{t}.npz")
        np.savez(
            score_filename, labels=self.all_label_scores, outputs=self.all_output_scores
        )
        self.args.logging.info(f"Save file to {score_filename}.")

    def test_param_change(self):
        save_dir = os.path.join("./outputs/results")
        os.makedirs(save_dir, exist_ok=True)

        self.old_model = copy.deepcopy(self.model)

        # exp_dir: /home/user/Codes/MAGR-github/outputs/user-fscl/class-mtl/magrpp05-buffer50-order12345
        base_dirname = os.path.dirname(self.args.exp_dir)
        # find all exp dirs with magrpp05-buffer50-order*
        exp_dirs = [
            os.path.join(base_dirname, d)
            for d in os.listdir(base_dirname)
            if os.path.isdir(os.path.join(base_dirname, d))
            and 'order' in d and self.args.model in d
        ]

        from utils.stat_res import read_res

        for exp_dir in exp_dirs:
            order = exp_dir.split("order")[-1]
            weight_path = os.path.join(exp_dir, "weights/last-task5.pth")
            if not os.path.exists(weight_path):
                self.args.logging.info(f"No weight file found at {weight_path}.")
                continue

            # the last train log file: logs/train-***.log
            log_files = [
                f
                for f in os.listdir(os.path.join(exp_dir, "logs"))
                if f.startswith("train-") and f.endswith(".log")
            ]
            log_files = sorted(log_files)
            if len(log_files) == 0:
                self.args.logging.info(f"No log file found at {os.path.join(exp_dir, 'logs')}.")
                continue
            
            log_file = os.path.join(exp_dir, "logs", log_files[-1])
            res = read_res(log_file)
            rho = res['rho'] / 100
            forgetting = res['forgetting'] / 100
            fwt = res['fwt'] / 100

            # load new model
            self.load_best_model(weight_path=weight_path)
            change = self._compute_param_change()

            self.args.logging.info(f"Order {order}, change: {change:.8f}")
            # write to file
            if self.args.local_rank < 1:
                with open(os.path.join(save_dir, "param_change.txt"), "a") as f:
                    f.write(f"{self.args.model}\t{order}\t{change:.8f}\t{rho:.6f}\t{forgetting:.6f}\t{fwt:.6f}\n")

    def _compute_param_change(self):
        change = 0
        total = 0
        for p_old, p_new in zip(
            self.old_model.state_dict().values(), self.model.state_dict().values()
        ):
            p_old = p_old.float().cpu()
            p_new = p_new.float().cpu()
            change += torch.norm(p_old - p_new).item()
            total += torch.norm(p_old).item()
        return change 
    ...
    def start(self):
        ...
        elif self.args.phase_task == "loss_landascape":
            self.test_w_loss_landascape()

        elif self.args.phase_task == "case_study":
            self.test_w_case_study()

        elif self.args.phase_task == "memory_feats":
            self.test_w_memory_feats()

        elif self.args.phase_task == "sample_feats":
            self.test_w_sample_feats()

        elif self.args.phase_task == "optimal_layer":
            self.test_w_optimal_layer()

        elif self.args.phase_task == "param_change":
            self.test_param_change()
