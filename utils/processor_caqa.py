...
class ProcessorAQA(Processor):
    ...
    def task_stat(self, logger):
        ...
        if self.args.wandb:
            if self.dataset.SETTING == "domain-il":
                d = {
                    "RESULT_mean_rhos": mean_rho[0],
                    **{f"RESULT_acc_{i}": a for i, a in enumerate(self.rhos)},
                }
            else:
                d = {
                    "RESULT_class_mean_rhos": mean_rho[0],
                    "RESUL.T_task_mean_rhos": mean_rho[1],
                    **{f"RESULT_class_rho_{i}": a for i, a in enumerate(self.rhos)},
                    **{
                        f"RESULT_task_rho_{i}": a
                        for i, a in enumerate(self.rhos_mask_classes)
                    },
                }
    ...
    def save_processed_results(self):
        ...
        for t in range(self.dataset.N_TASKS):
            # load i-th task weight
            weight_path = os.path.join(
                os.path.dirname(self.args.weight_save_path), f"last-task{t}.pth"
            )
            state = torch.load(weight_path)
            self.model.load_state_dict(state["model"], strict=False)
            ...
            # print(self.all_output_scores.shape, self.all_output_scores.shape)
            np.savez(
                score_filename,
                labels=self.all_label_scores,
                outputs=self.all_output_scores,
            )
            self.args.logging.info(f"Save file to {score_filename}.")

    def save_joint_features(self):
        save_dir = os.path.join(self.args.exp_dir, "results")
        os.makedirs(save_dir, exist_ok=True)
        ...
        # i3d feats
        i3d_feats = self.model.get_features(self.dataset)
        ...
        self.model.net.apply(weights_init)
        raw_feats = self.model.get_features(self.dataset)
        # best feats
        self.load_best_model(weight_path=self.args.weight_path)
        best_feats = self.model.get_features(self.dataset)
        # save feats
        feats_filename = os.path.join(save_dir, f"joint_feats.npz")
        np.savez(feats_filename, raw=raw_feats, i3d=i3d_feats, best=best_feats)
        self.args.logging.info(f"Save file to {feats_filename}.")
