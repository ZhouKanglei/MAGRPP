...
class Processor(object):
    ...
    def evaluate(self, dataset, last=False):
        ...
        for k, test_loader in enumerate(dataset.test_loaders):
            ...
            self.accs.append(
                correct / total * 100 if "class-il" in self.model.COMPATIBILITY else 0
            )
        ...
        for t in range(self.dataset.N_TASKS):
            ...
            if t:
                ...
                if self.dataset.SETTING == "class-il":
                    self.results_mask_classes[t - 1] = (
                        self.results_mask_classes[t - 1] + self.accs_mask_classes
                    )
            # sequential learning
            for epoch in range(self.args.n_epochs):
                ...
                if self.args.local_rank < 1:
                    ...
                    self.args.logging.info(
                        f"Save model state to {self.args.weight_path}."
                    )

            if self.args.model != "joint" and self.args.local_rank < 1:
                print()
            # ending task
            if hasattr(self.model, "end_task"):
                self.model.end_task(self.dataset)
                if self.args.local_rank < 1 and t == self.dataset.N_TASKS - 1:
                    print()
            # evaluation
    ...
    def task_stat(self, logger):
        ...
        if self.dataset.SETTING == "domain-il":
            ...
            self.args.logging.info(
                f"Accuracy [Class-IL]: {mean_acc[0]:.2f}%, "
