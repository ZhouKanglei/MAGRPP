...
class ClassMTL(ContinualDataset):
    ...
    def __init__(self, args):
        ...
        if self.args.task_order is not None:
            task_order = self.args.task_order
            assert len(task_order) == self.N_TASKS, "The length of task order must be equal to the number of tasks."

            self.train_sample_splits = [self.train_sample_splits[i] for i in task_order]
            self.test_sample_splits = [self.test_sample_splits[i] for i in task_order]
