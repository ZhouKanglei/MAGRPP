...
class I3D(torch.nn.Module):
    ...
    def get_feats(self, inp):
        feats = {}
        x = self.conv3d_1a_7x7(inp)
        feats["conv3d_1a_7x7"] = x
        x = self.maxPool3d_2a_3x3(x)
        feats["maxPool3d_2a_3x3"] = x
        x = self.conv3d_2b_1x1(x)
        feats["conv3d_2b_1x1"] = x
        x = self.conv3d_2c_3x3(x)
        feats["conv3d_2c_3x3"] = x
        x = self.maxPool3d_3a_3x3(x)
        feats["maxPool3d_3a_3x3"] = x

        x = self.mixed_3b(x)  # <-- human-centric supervision here
        feats["mixed_3b"] = x
        ...
        x = self.mixed_4e(x)
        feats["mixed_4e"] = x
        x = self.mixed_4f(x)  # <-- dynamics/action alignment here
        feats["mixed_4f"] = x

        x = self.maxPool3d_5a_2x2(x)
        feats["maxPool3d_5a_2x2"] = x
        x = self.mixed_5b(x)
        feats["mixed_5b"] = x
        x = self.mixed_5c(x)
        feats["mixed_5c"] = x

        out = self.avg_pool(x)  # final AQA feature (to regressor)
        feats["avg_pool"] = out
...
if __name__ == "__main__":
    ...
    # Forward pass
    feat_shallow, feat_mid, feat_deep = model(sample_input, return_feats=True)
    print("Shallow feature shape:", feat_shallow.shape)
    print("Mid feature shape:", feat_mid.shape)
    print("Deep feature shape:", feat_deep.shape)
